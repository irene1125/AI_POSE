...
class CustomDataset(Dataset):
    ...
    def data_lodaer(self): # loader!!!!!!!오타!!!!!!!!!!!!!!!!!!!!!!!!
        print("working well!!")
        ...
        if os.path.isfile(os.path.join(self.data_dir, self.mode, self.mode + '.pt')):
            ...
            for (roots, dirs, files) in os.walk(os.path.join(self.data_dir, self.mode, 'images')):
                if len(files) != 0:
                    camera_path = os.path.join(self.data_dir, self.mode, 'camera', roots.split('/')[-1] + '.json') # windows가 아니여서 '/'처리
                    ...
                    for file in files:
                        annot_path = os.path.join(self.data_dir, self.mode, 'labels', roots.split('/')[-1][:-2], # windows가 아니여서 '/'처리
                                                  '3D_' + file.split('_')[0] + '_' + file.split('_')[1] + '_' +
                                                  file.split('_')[3][:-3] + 'json')
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.joint_num): #비트연산자 OR일 때 \
            joint_vis[i] *= (
                    (joint_img[i, 0] >= 0) & \
...
class TestDataset(Dataset):
    ...
    def data_loader(self):
        ...
        if os.path.isfile(os.path.join(self.data_dir, 'test', 'test' + '.pt')):
            db = torch.load(os.path.join(self.data_dir, 'test', 'test' + '.pt'))
        else:
            db = []
            for (roots, dirs, files) in os.walk(os.path.join(self.data_dir, 'test/images')):
                if (roots.split('/')[-1] == '.ipynb_checkpoints'):
                    continue
                if len(files) != 0:
                    print(roots)
                    print(dirs)
                    print(files)
                    camera_path = os.path.join(self.data_dir, 'test', 'camera', roots.split('/')[-1] + '.json')
                    print(camera_path)
                    camera = pd.read_json(camera_path)
                    print(camera)
                    ...
                    if len(db) % 100 == 0:
                        print(len(db))
            torch.save(db, os.path.join(self.data_dir, 'test', 'test' + '.pt'))
        return db
