...
DATA_DIR = os.path.join(ROOT_PROJECT_DIR, 'DATA') # 서현수정
TRAIN_DATA_DIR = os.path.join(DATA_DIR, 'split/task04_train') # 서현수정
PROJECT_DATA_DIR = os.path.join(DATA_DIR, 'train_val') # split 들어가는 곳
#PROJECT_DATA_DIR = os.path.join(DATA_DIR, 'sample') # split 들어가는 곳
print(DATA_DIR) # 확인용
...
if __name__ == '__main__':
    ...
    train_dataset = CustomDataset(data_dir=PROJECT_DATA_DIR, mode='train', input_shape=INPUT_SHAPE, output_shape=OUTPUT_SHAPE)
    validation_dataset = CustomDataset(data_dir=PROJECT_DATA_DIR, mode='val', input_shape=INPUT_SHAPE, output_shape=OUTPUT_SHAPE)
    ...
    #OneCycleLR 바꿀 예정 , CosineAnnealingLR, StepLR
    #Adam , WAdam
    ...
    for epoch_index in tqdm(range(EPOCHS)):
        ...
        print('2')
        print(trainer.train_mean_loss) # ja
